...
#' SMR <- y/E
#' ## Plot SMR
#' plotmap(SMR, scotland_sf, nclr=9, location="topleft")
plotmap <- function(values, map, log=FALSE, nclr=7, include.legend=TRUE, lwd=0.5, round=3,
  ...
  colornum <- findInterval(values, brks, all.inside=TRUE)
  ...
  plot(sf::st_geometry(map), axes=TRUE, col=colcode, lwd=lwd)
  if (include.legend) {
