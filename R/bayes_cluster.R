...
#' @param spatial.obj an sf object representing the study region
...
#' cases <- cases[-remove]
#' spatial.obj <- NYleukemia_sf[-remove, ]
...
#' #output <- bayes_cluster(y, E, population, spatial.obj, centroids, max.prop,
#' #  shape, rate, J, pi0, n.sim.lambda, n.sim.prior, n.sim.post)
#' #plotmap(output$prior.map$high.area, spatial.obj)
#' #plotmap(output$post.map$high.area, spatial.obj)
#' #plotmap(output$post.map$RR.est.area, spatial.obj, log=TRUE)
#' #barplot(output$pk.y, names.arg=0:J, xlab="k", ylab="P(k|y)")    
bayes_cluster <-
  function(y, E, population, spatial.obj, centroids, max.prop, shape, rate, J, pi0,
    ...
    geo.objects <- create_geo_objects(max.prop, population, centroids, spatial.obj)
